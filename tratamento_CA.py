...
conteudo_colunas = ["Conteúdo", "Título do vídeo", "Horário de publicação do vídeo", "Duração", "Espectadores únicos", "Comentários adicionados", "Compartilhamentos", 'Marcações "Gostei"', "RPM (USD)", "Taxa de cliques de impressões (%)", "Impressões", "Inscritos", "Porcentagem visualizada média (%)", "Visualizações", "Tempo de exibição (horas)", "Duração média da visualização", "Receita estimada (USD)"]
...
def completar_data(artista):
    """
    Completa os dados de data em arquivos CSV, ignorando arquivos com
    prefixos específicos ('videos_', 'lives_', 'shorts_').
    """
    ...
    for arquivo in arquivos_csv:
        ...
        if nome_do_arquivo.startswith(('videos_', 'lives_', 'shorts_')):
            continue 
